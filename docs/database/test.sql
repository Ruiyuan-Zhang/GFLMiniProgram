...
DROP TABLE IF EXISTS tb_dataFormata;
DROP TABLE IF EXISTS tb_globalModel;
DROP TABLE IF EXISTS tb_clientModel;

CREATE TABLE tb_category (
    id INT(11) UNIQUE NOT NULL COMMENT '分类编号',
    name VARCHAR(30) DEFAULT '' COMMENT '联邦学习分类名称',
    description VARCHAR(2000) DEFAULT '' COMMENT '分类介绍',
    file VARCHAR(30) DEFAULT '' COMMENT '封面图片地址'
)DEFAULT CHARSET=utf8;

INSERT INTO tb_category VALUES (1, '机器学习','机器学习是一门多领域交叉学科，涉及概率论、统计学、逼近论、凸分析、算法复杂度理论等多门学科。专门研究计算机怎样模拟或实现人类的学习行为，以获取新的知识或技能，重新组织已有的知识结构使之不断改善自身的性能。','images/category/1.webp');
...
INSERT INTO tb_category VALUES (3, '语音识别','语音识别是一门交叉学科。近二十年来，语音识别技术取得显著进步，开始从实验室走向市场。人们预计，未来10年内，语音识别技术将进入工业、家电、通信、汽车电子、医疗、家庭服务、消费电子产品等各个领域。 语音识别听写机在一些领域的应用被美国新闻界评为1997年计算机发展十件大事之一。很多专家都认为语音识别技术是2000年至2010年间信息技术领域十大重要的科技发展技术之一。 语音识别技术所涉及的领域包括：信号处理、模式识别、概率论和信息论、发声机理和听觉机理、人工智能等等。','images/category/3.webp');
...
CREATE TABLE tb_task (
    taskId INT(11) UNIQUE NOT NULL COMMENT '任务编号',
    categoryId INT(11) NOT NULL COMMENT '分类编号',
    name VARCHAR(30) DEFAULT '' COMMENT '联邦学习任务名称',
    description VARCHAR(2000) DEFAULT '' COMMENT '联邦学习描述',
    file VARCHAR(30) DEFAULT '' COMMENT '封面图片地址',
    initModelFile VARCHAR(30) DEFAULT '' COMMENT '初始化文件地址',
    superParams VARCHAR(1000) DEFAULT '' COMMENT '超参数设置',
    maxTimesPerClient INT(11) DEFAULT 2 COMMENT '每个客户端可以参与的训练的次数'
)DEFAULT CHARSET=utf8;

CREATE TABLE tb_dataFormata (
    taskId INT(11)  NOT NULL COMMENT '任务编号',
    name VARCHAR(30) DEFAULT '' COMMENT '数据名称',
    type VARCHAR(30) DEFAULT '' COMMENT '数据类型',
    size VARCHAR(30) DEFAULT '28 28' COMMENT 'image数据尺寸'
)DEFAULT CHARSET=utf8;

CREATE TABLE tb_globalModel (
    globalModelId INT(11) UNIQUE NOT NULL COMMENT '全局模型编号',
    lastGlobalModel INT(11) COMMENT '延续的上一个全局模型',
    taskId INT(11) NOT NULL COMMENT '任务名称',
    file VARCHAR(30) DEFAULT '' COMMENT '全局模型地址',
    clientModelIds VARCHAR(1000) DEFAULT '' COMMENT '所引用的客户端模型'
)DEFAULT CHARSET=utf8;

CREATE TABLE tb_clientModel (
    globalModelId INT(11) NOT NULL COMMENT '全局模型编号',
    clientModelId INT(11) UNIQUE NOT NULL COMMENT '客户端模型编号',
    taskId INT(11)  NOT NULL COMMENT '任务编号',
    file VARCHAR(30) DEFAULT '' COMMENT '全局模型地址'
)DEFAULT CHARSET=utf8;
